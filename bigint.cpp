...
//---------------------------------------------------------------------
const LL B = 10000;      // base (power of 10)
...
BigNum operator*(BigNum x, LL a) {
    for (int i = 0; i < x.size; ++i)
        x.digit[i] *= a;
    return normal(x);
}
pair<BigNum, LL> divmod(BigNum x, LL a) {
    LL c = 0, t;
    for (int i = x.size-1; i >= 0; --i) {
        t                    = B * c + x.digit[i];
        x.digit[i] = t / a;
    ...
    return pair<BigNum, LL>(normal(x), c);
}
BigNum operator/(BigNum x, LL a) {
    return divmod(x, a).first;
}
LL operator%(BigNum x, LL a) {
